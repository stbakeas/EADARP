...
int main() {
	...
	inst.RandomInit(100,20,5, 20, 20, 1440, 15);
	//HeterogeneousDARP();

	Solution my_solution = algorithms::details::constructInitialSolution(100);

	for (auto& my_route : my_solution.routes) {
		cout << "Route ID: " << my_route.first->id << " with capacity " << my_route.first->capacity << " : ";
		for (size_t i = 0; i < my_route.second.sequence.size(); i++)
		{
			cout << my_route.second.battery[i]
				<< "->";
		}
		cout << "\n";
		my_solution.total_cost += my_route.second.Evaluate();
	}
	cout << "Total Cost of my solution = " << my_solution.total_cost << "\n";
	cout << "Number of rejected requests of my solution = " << my_solution.rejected.size() << "\n";
